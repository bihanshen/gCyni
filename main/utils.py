...
from naima.models import ExponentialCutoffPowerLaw, BrokenPowerLaw, PionDecay, InverseCompton, Synchrotron
...
def _gamma_index_from_model(model, distance, e1=0.5 * u.TeV, e2=2.0 * u.TeV):
    """
...
def plot_hadronic_model(
    ...
    data_dict : dict, optional
        Optional data dictionary containing:
        {"energy": ..., "flux": ..., "flux_err": ...}
        All must be `astropy.units.Quantity`.
    show : bool
    ...
    gamma_med = _gamma_index_from_model(PP_med, distance)
    gamma_low = _gamma_index_from_model(PP_low, distance)
    gamma_high = _gamma_index_from_model(PP_high, distance)

    Ebreak_med, glow_med, ghigh_med = estimate_gamma_break_from_model(PP_med, distance=distance)
    Ebreak_low, glow_low, ghigh_low = estimate_gamma_break_from_model(PP_low, distance=distance)
    Ebreak_high, glow_high, ghigh_high = estimate_gamma_break_from_model(PP_high, distance=distance)
    ...
    print(
        f"Gamma-ray index @1 TeV: {gamma_med:.3f} "
        f"(-{(gamma_med - gamma_low):.3f}, +{(gamma_high - gamma_med):.3f})"
    )
    ...
    print(
        f"Local gamma-ray indices: low={glow_med:.3f}, high={ghigh_med:.3f}"
    )
...
def plot_leptonic_model(
    ...
    """
    Plot leptonic (Inverse Compton) SED (BPL or ECPL) and optionally observed data.
    ...
    seed_photon_fields : list of str, optional
        Seed photon fields for IC (e.g. ["CMB"], ["CMB","IR","FIR"]).
        If None, defaults to ["CMB"].
    ax : matplotlib.axes._subplots.AxesSubplot, optional
        Axis to plot on (creates new one if None).
    label : str
        Label for the model curve.
    ...
    alpha : float
        Transparency for the model curve.
    data_dict : dict, optional
        Dictionary of observational datasets to overplot, e.g.:
            {"VERITAS (VEGAS)": veritas_vegas_table, ...}
        Each table should have columns compatible with `plot_data()`
        (energy, flux, flux_error[/_lo/_hi], ul).
    show : bool
        If True, calls plt.show() at the end.
    ...
    spec_med, IC_med = build_leptonic_model_local(p_med)
    spec_low, IC_low = build_leptonic_model_local(p_low)
    spec_high, IC_high = build_leptonic_model_local(p_high)

    Eref = np.array([1.0]) * u.TeV

    dnde_med = IC_med.flux(Eref, distance=distance)[0].to("1/(TeV cm2 s)")
    dnde_low = IC_low.flux(Eref, distance=distance)[0].to("1/(TeV cm2 s)")
    dnde_high = IC_high.flux(Eref, distance=distance)[0].to("1/(TeV cm2 s)")

    sed_med = IC_med.sed(Eref, distance=distance)[0].to("TeV cm-2 s-1")
    sed_low = IC_low.sed(Eref, distance=distance)[0].to("TeV cm-2 s-1")
    sed_high = IC_high.sed(Eref, distance=distance)[0].to("TeV cm-2 s-1")

    gamma_med = _gamma_index_from_model(IC_med, distance)
    gamma_low = _gamma_index_from_model(IC_low, distance)
    gamma_high = _gamma_index_from_model(IC_high, distance)

        # ---- Estimate gamma-ray break directly from modeled IC spectrum ----
    Ebreak_med, gamma_low_fit, gamma_high_fit = estimate_gamma_break_from_model(
        IC_med,
        distance=distance,
        energy_range=(1e-4, 1e2),   # adjust if needed
        ngrid=500,
        use_sed=False,              # recommended
    )

    Ebreak_low, _, _ = estimate_gamma_break_from_model(
        IC_low,
        distance=distance,
        energy_range=(1e-4, 1e2),
        ngrid=500,
        use_sed=False,
    )
    ...
    print(
        f"Local gamma-ray indices from IC model: "
        f"low={gamma_low_fit:.3f}, high={gamma_high_fit:.3f}"
    )
